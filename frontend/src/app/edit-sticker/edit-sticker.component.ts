...
export class EditStickerComponent implements OnInit {
  ...
  public new_sticker : Node;
  onSubmit(){
    this.new_sticker =  {node_id: this.node.node_id,
    ...
    if(this.new_sticker.node_id != null){
      this.api.updateSticker(this.new_sticker).then((r:any)=>{
        this.posted.emit(true)
      });
    } else{
      this.api.newSticker(this.new_sticker).then((r:any)=>{
        this.posted.emit(true)
      });
    }
  }
